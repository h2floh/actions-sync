...
func (f *PullOnlyFlags) Init(cmd *cobra.Command) {
	cmd.Flags().StringVar(&f.Token, "source-token", "", "Token to access private/internal repos")
...
func Pull(ctx context.Context, flags *PullFlags) error {
	...
	return PullManyWithGitImpl(ctx, flags.SourceURL, flags.CacheDir, flags.Token, repoNames, gitImplementation{})
}

func PullManyWithGitImpl(ctx context.Context, sourceURL, cacheDir string, token string, repoNames []string, gitimpl GitImplementation) error {
	for _, repoName := range repoNames {
		if err := PullWithGitImpl(ctx, sourceURL, cacheDir, token, repoName, gitimpl); err != nil {
			return err
		}
	}
	return nil
...
func PullWithGitImpl(ctx context.Context, sourceURL, cacheDir string, token string, repoName string, gitimpl GitImplementation) error {
	...
	// create BasicAuth if optional token was provided
	var auth *http.BasicAuth = nil
	if len(token) > 0 {
		auth = &http.BasicAuth{
			Username: "actions-sync",
			Password: token,
		}
	}
